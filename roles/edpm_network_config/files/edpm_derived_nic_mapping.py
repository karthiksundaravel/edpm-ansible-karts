...
#
# Reads and writes /etc/edpm/derived_nic_mapping.yaml (configurable via -f).
# YAML shape:
#   interface_mapping:
...
DEFAULT_PATH = "/etc/edpm/derived_nic_mapping.yaml"
...
def _is_available_nic(interface_name: str, check_active: bool) -> bool:
    if interface_name == "lo":
        return False
    if not _is_real_nic(interface_name):
        return False
    if check_active:
        try:
            with open(
...
def load_yaml(path: str) -> Dict[str, str]:
    ...
    try:
        ...
        raise RuntimeError("PyYAML is required (python3-pyyaml).") from exc
...
def save_yaml(path: str, mapped: Dict[str, str]) -> None:
    import yaml
